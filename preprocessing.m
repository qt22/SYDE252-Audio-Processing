...
function [x, x_mono, x_mono_16k, fs, num_x, new_fs, num_x_16k, plot_num] = preprocessing(infile, outfile, plot_num)
    ...
    pause(num_x/fs); % let the current audio file finish first before playing the resampled audio
    ...
    sound(x_mono_16k, new_fs); % play resampled file
    pause(num_x_16k/new_fs); % let the resampled file finish playing before playing the next input
end